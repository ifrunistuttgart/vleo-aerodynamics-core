...
for body_index = 1:num_bodies
    ...
    % Compute triangle areas: 0.5 * norm of each normal vector (before normalization)
    areas = 0.5 * normal_norms;
    
    % Create structure for current body (initially in CAD frame)
    bodies{body_index} = struct('vertices_B', vertices_CAD, ...  % Will be transformed later
                               'centroids_B', centroids_CAD, ...  % Will be transformed later
                               'normals_B', normals_CAD, ...      % Will be transformed later
                               'areas', areas);
...
% %
% % Usage:
% % satellite = import_gmsh('C:/path/to/mesh/full_satellite.m', body_index);
% %
% % The input file must define a variable 'msh' with fields:
% % - msh.POS: Node positions (n_nodes × 3)
% % - msh.TRIANGLES: Element definitions (n_elements × 4), where
...
% % Inputs:
% % - relative_path: Path to the .m file containing the msh structure
% % - body_index: Index of the body to extract (1-based indexing)
% %
% % Output:
% % body_data struct with:
% % - vertices_B: 3×3×n_triangles array for the specified body
% % - centroids_B: 3×n matrix of triangle centroids for the specified body
% % - normals_B: 3×n matrix of normal vectors for the specified body
% % - areas: 1×n vector of triangle areas for the specified body
% % ========================================================================
% function body_data = import_gmsh(relative_path, body_index)
% 
% % Convert to char if it's a string to ensure compatibility
% if isstring(relative_path)
%     relative_path = char(relative_path);
...
% 
% % Validate body_index
% if body_index < 1 || body_index > length(body_ids)
%     error('Body index %d is out of range. Available bodies: 1 to %d', ...
%           body_index, length(body_ids));
% end
% 
% % Get the body ID for the requested index
% target_body_id = body_ids(body_index);
% 
% % Extract vertices for the specific body
% body_triangles = msh.TRIANGLES(msh.TRIANGLES(:,4) == target_body_id, 1:3);
% vertices_B = reshape(msh.POS(reshape(body_triangles', [], 1), :)', ...
%                     [3, 3, size(body_triangles, 1)]);
% 
% % Compute triangle centroids (mean of each triangle's 3 vertices)
% centroids_B = reshape(mean(vertices_B, 2), 3, []);
% 
% % Compute normal vectors for each triangle: cross(v2−v1, v3−v1)
% normals_B = reshape(cross(vertices_B(:,2,:) - vertices_B(:,1,:), ...
%                          vertices_B(:,3,:) - vertices_B(:,1,:)), 3, []);
% 
% % Compute triangle areas: 0.5 * norm of each normal vector
% areas = 0.5 * sqrt(sum(normals_B.^2, 1));  %areas instead of areas_B
% 
% % Create output structure for single body
