...
for i = 1:num_bodies
    ...
    % Get the number of faces for this body
    num_faces = size(X, 2);
    
    % Plot the body - handle scalar values properly
    if isempty(current_scalar_values)
        C = 'green';
    else
        % Ensure scalar values match the number of faces for this body
        if length(current_scalar_values) == num_faces
            C = current_scalar_values;
        elseif length(current_scalar_values) == 1
            % If only one scalar value, replicate it for all faces
            C = repmat(current_scalar_values, 1, num_faces);
        else
            % If mismatch, use default color and warn
            warning('Scalar values size (%d) does not match number of faces (%d) for body %d. Using default color.', ...
                    length(current_scalar_values), num_faces, i);
            C = 'green';
        end
    end
    ...
    if ~isempty(current_vectorial_values)
        % Check if vectorial values dimensions match the number of face centroids
        if size(current_vectorial_values, 2) == size(centroids_B, 2)
            ...
            warning('Vectorial values size (%d) does not match number of centroids (%d) for body %d. Skipping vectorial plot.', ...
                    size(current_vectorial_values, 2), size(centroids_B, 2), i);
        end
    end
